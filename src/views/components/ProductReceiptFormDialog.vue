...
import { addProductReceiptHeader, getWarehouseTree, generateCode, getProductDetail, exportExcel } from '~/api'
...
function handleQr (row: { materialName: string; batchCode: string; quantityReceipt: any; }) {
  exporting.value = true
  const fileName = row.materialName + '-' + row.batchCode + '二维码'
  exportExcel('/api/MaterialReceiptRow/GenQrCodeForGY?', fileName, {
...
function getProductData () {
  getProductDetail({ id: erweimaData.value.productId })
    .then(({ data }) => {
      ...
      formData.value.productReceiptRows[0].whsLocationCode = data.whsLocationCode
      formData.value.productReceiptRows[0].whsLocationName = data.whsLocationName
      formData.value.productReceiptRows[0].unitOfMeasure = data.unitOfMeasure
      formData.value.productReceiptRows[0].warehouseALs = [data.warehouseId, data.areaId, '']
    })
    .catch(() => {
    ...
    .finally(() => {
      console.log('formData.value.productReceiptRows[0].warehouseALs', formData.value.productReceiptRows[0].warehouseALs)
    })
}
/**
 * 获取仓库树NV
...
function handleChange (val:any, row: { warehouseId: string; warehouseCode: string; warehouseName: string; whsAreaId: string; whsAreaCode: string; whsAreaName: string; whsLocationId: string; whsLocationCode: string; whsLocationName: string; }) {
  if (val != null && val.length > 0) {
    warehouseOptions.value.forEach((item: { value: any; code: null; name: any; children: any[] | null; }) => {
      if (item.value === val[0]) {
      ...
      if (item.children != null && item.children.length > 0) {
        item.children.forEach((sitem: { value: any; code: any; name: any; children: any[] | null; }) => {
          ...
          if (sitem.children != null && sitem.children.length > 0) {
            sitem.children.forEach((titem: { value: any; code: any; name: any; }) => {

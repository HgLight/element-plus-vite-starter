...
mouldId: [{
  ...
  validator: (_rule: any, value: string, callback: (arg0?: Error | undefined) => void) => {
    if (formData.value.processName.indexOf('成型') !== -1 && formData.value.processName.indexOf('成型工检') === -1 && value === '') {
...
quantityQualified: [
  { required: true, trigger: 'blur', message: '请输入产出数量' },
  {
    ...
    validator: (_rule: any, value: any, callback: (arg0?: Error | undefined) => void) => {
      ...
      } else if (formData.value.unlockUserId === '00000000-0000-0000-0000-000000000000' && (Math.abs(value - formData.value.quantity) / formData.value.quantity) * 100 > quantityQualifiedDeviation.value) {
        callback(new Error('产出数量超过或低于百分之' + quantityQualifiedDeviation.value + ',需要解锁。'))
      } else {
        callback()
      }
    }
...
watch(()=>props.defaultQuantityNum,(val)=>{
  quantityNum.value = val
},
{immediate: true}
)
watch(()=>props.workTask,(val)=>{
  formData.value = val
},
{immediate: true}
)
   ...
   function getOperatorListData () {
     ...
     */
     function handlePick () {
      ...
      pickingDialogRef.value.show(formData.value, quantityNum.value,isBeforehand.value)
    }
    /**
     * 入库事件
     * @param {*} formData
     ...
     function modify (isFinish?:boolean) {
      ...
      return new Promise((resolve, reject) => {
        saveWorkFeedback({
          ...
          workstationId: formData.value.workstationId ? formData.value.workstationId : '00000000-0000-0000-0000-000000000000', // 工作站ID
  ...
  <div>
    ...
    >
      <el-row :gutter="40">
          ...
          <el-form-item>
            <div class="w-full flex justify-center">
              <el-button
                ...
                :title="erweimaData.taskStatus==3||erweimaData.taskStatus==4?'工单已暂停或已结束':previousTask&&previousTask.status!=4?'上一个工序未结束，不可开始。':formData.status === 2|| formData.status === 4?formData.statusName +'，不可开始':formData.issueStatus === 2?'请先领料':''"
                @click="handleStart()"
              >
                开始
              </el-button>
              <el-button
                ...
                :title="erweimaData.taskStatus==3||erweimaData.taskStatus==4?'工单已暂停或已结束':formData.status === 4|| formData.status === 1?formData.statusName +'，不可结束':formData.issueStatus === 2?'请先领料':''"
                @click="handleEnd()"
              >
                结束
              </el-button>
              ...
              <el-button
                ...
                type="primary"
                :disabled="erweimaData.taskStatus==3||erweimaData.taskStatus==4||formData.status!==4||!formData.quantityQualified"
                :title="erweimaData.taskStatus==3||erweimaData.taskStatus==4?'工单已暂停或已结束':formData.status!==4?'未结束，不可入库':!formData.quantityQualified?'产出数量为0，不可入库':''"
                @click="handleReceipt()"
              >
                入库
              </el-button>
          ...
          >
            ...
            <template v-else>
              <el-select
                ...
                :title="formData.status===1?'未开始不可编辑':formData.status===4?'已完成不可编辑':''"
        ...
        <ReColForm v-if="formData.processName.indexOf('成型') !== -1 && formData.processName.indexOf('成型工检') === -1">
          <el-form-item
            label="模具"
            prop="mouldId"
          >
        ...
        <ReColForm>
          <el-form-item
            label="产出数量"
            prop="quantityQualified"
          >
            <el-input
              v-model="formData.quantityQualified"
              :disabled="formData.status===1||formData.status===4||formData.unlockUserId !== '00000000-0000-0000-0000-000000000000'"
              :title="formData.status===1?'未开始不可编辑':formData.status===4?'已完成不可编辑':formData.unlockUserId !== '00000000-0000-0000-0000-000000000000'?'解锁后无法修改产出数量':''"
              @change="(val)=>{
                formData.quantityQualified=val?isNaN(parseFloat(val))?0:parseFloat(val):0
              }"
            >
              <template
                v-if="false"
              ...
              <template #append>
                <el-button
                  type="primary"
                  :disabled="erweimaData.taskStatus==3||erweimaData.taskStatus==4||formData.status===1||formData.status===4"
                  :title="erweimaData.taskStatus==3||erweimaData.taskStatus==4?'工单已暂停或已结束':formData.status===1?'未开始，不可解锁':formData.status===4?'已完成，不可解锁':''"
                  @click="handleUnlock()"
                >
                  解锁
                </el-button>

...
const formRules={
  defectId: [
    { required: true, message: '请选择不良品', trigger: 'change' }
  ],
  quantityUnqualified: [
    { required: true, trigger: 'blur', message: '请输入数量' },
    {
      trigger: 'blur',
      validator: (rule: any, value: string | number, callback: (arg0?: Error | undefined) => void) => {
        if (value === '') {
...
function getDefectList (productLineType: any) {
  getDefectListNv({ sysOwnerCpy: '', state: '正常', productLineType }).then((res) => {
        ...
        <el-col :span="12">
          ...
          >
            <el-input
              ...
              autocomplete="off"
              @change="(val)=>{
                formData.quantityUnqualified=val?isNaN(parseInt(val))?0:parseInt(val):0
              }"
            />
          </el-form-item>
        </el-col>

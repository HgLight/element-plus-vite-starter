 ...
 * 结束任务事件
 */
function handleEnd(workTask: {
  checkStatus: number;
  quantityQualified: any;
  userId: any;
  taskId: any;
  quantityTask: number;
  ...
  <div>
    ...
    >
      <el-tab-pane
        ...
        stretch
        :label="
          workTask.processName +
          '---' +
          workTask.statusName +
          '---' +
          workTask.workshopName
        "
        :data="workTask"
      ...
      >
        <div v-if="currentWorkTask.taskName === workTask.taskName">
          <div v-if="workTask.checkStatus !== 1">
            <el-row :gutter="40">
              <ReCol24 style="margin-bottom: 22px">
                <el-button
                ...
                <el-button
                  v-if="
                    ...
                    (workTask.processName.indexOf('先行') != -1 ||
                      workTask.processName.indexOf('首件') != -1)
                  "
                  type="primary"
                  :loading="exporting"
              ...
              <ReCol24>
                <CheckReportTable :key="now" :current-task="workTask" />
              </ReCol24>
            </el-row>
          </div>
          <div v-else>
            <div :key="currentWorkTask.id">
              <WorkTaskForm
                :key="workTask.taskId"
    ...
    <el-empty v-else description="暂无生产任务数据" />
    <DefectFormDialog ref="defectDialogRef" @done="getWorkFeedbackDefectList" />
  </div>
</template>

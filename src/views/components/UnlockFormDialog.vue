...
watch(()=>props.quantityQualifiedDeviation,(val)=>{
  deviation.value = val
},{
  immediate:true
})
...
function loginNameValidator (rule: any, value: any, callback: (arg0?: Error | undefined) => void) {
...
function passwordValidator (rule: any, value: any, callback: (arg0?: Error | undefined) => void) {
...
function ccSlValidator (rule: any, value: any, callback: (arg0?: Error | undefined) => void) {
  if (value === '') {
    callback(new Error('请输入产出数量'))
  } else if (value <= 0) {
    callback(new Error('产出数量不能小于等于0!'))
  } else if ((Math.abs(value - workTask.value.quantity) / workTask.value.quantity) * 100 <= deviation.value) {
  ...
  formRef.value.validate((valid: any) => {
    if (valid) {
      ...
      saveWorkFeedback({
        ...
        machineryId: val.machineryId ? val.machineryId : '00000000-0000-0000-0000-000000000000', // 设备ID
        ...
        <el-col :span="12">
          ...
          >
            <el-input-number
              ...
              @change="(val:any)=>{
                formData.ccSl=val?isNaN(parseFloat(val))?0:parseFloat(val):0
              }"
            />
          </el-form-item>
        </el-col>

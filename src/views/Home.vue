...
const {workOrderCode,erweimaData,workTaskFeedbacks,currentWorkTask}=storeToRefs(useAppStoreHook())
...
function submit () {
  ...
  loadWorkOrder().then(() => {
    if (erweimaData.value !== null && erweimaData.value.workOrderStatus === 2) {
      dialogVisible.value = true
    }
  }).catch(() => {
  }).finally(() => {
    if (erweimaData.value !== null && (erweimaData.value.workOrderStatus === 3 || erweimaData.value.workOrderStatus === 1)) {
      ElMessage.warning(erweimaData.value.workOrderStatus === 3 ? '该工单已关闭' : '该工单还未提交',);
      scanDialogRef.value.close(0, QRcodeVal.value)
    } else {
      scanDialogRef.value.close(erweimaData.value == null || JSON.stringify(erweimaData.value) === '{}' ? 0 : 1, QRcodeVal.value)
    }
  })
}
function startHome () {
 ...
 *刷新未读信息Handler
  ...
  */
  ...
  if (QRcodeVal.value === data) {
    console.log('QRcodeVal.value == data.workOrderCode', QRcodeVal.value === data)
    submit()
  }
}
